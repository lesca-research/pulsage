...
if isRaw
    ...
    switch lower(ImportOptions.ImportMode)
        ...
        case 'epoch'
            ...
            for ieph = 1:length(ImportOptions.iEpochs)
                ...
                if isfield(sFile.epochs(iEpoch), 'timestamp')
                    BlocksToRead(end).Timestamp = sFile.epochs(iEpoch).timestamp;
                end
        ...
        case 'time'
            ...
            for iBlock = 1:nbBlocks
                ...
                BlocksToRead(end).TimeOffset = 0;
                if isfield(sFile, 'timestamp')
                    BlocksToRead(end).Timestamp = sFile.timestamp;
                end
    ...
    for iFile = 1:length(BlocksToRead)
        ...
        if isfield(BlocksToRead, 'Timestamp')
            DataMat.Timestamp = BlocksToRead(iFile).Timestamp;
        end
        ...
        if isfield(sFile.header, 'units')
            DataMat.AllUnits = sFile.header.units;
        end
