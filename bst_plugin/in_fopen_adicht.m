...
hdr.units = cellfun(@(x) x{1} , {objFile.channel_specs(:).units} , 'UniformOutput' , false);

comments = objFile.getAllComments();
...
if (hdr.nEpochs > 1)
    ...
    for i = 1:hdr.nEpochs
        ...
        dstart = objFile.records(i).data_start_str;
        if ~isempty(dstart)
            dt = datetime(dstart, 'InputFormat', 'dd-MMM-yyyy HH:mm:ss');
            timestamp = replace(replace(replace(char(dt, 'yyyy_MM_dd_HH--mm-ss.'), '--', 'h'), '-', 'm'), '.', 's');
            sFile.epochs(i).timestamp = timestamp;
        end
...
% Single record
    ...
    dstart = objFile.records(1).data_start_str;
    if ~isempty(dstart)
        dt = datetime(dstart, 'InputFormat', 'dd-MMM-yyyy HH:mm:ss');
        timestamp = replace(replace(replace(char(dt, 'yyyy_MM_dd_HH--mm-ss.'), '--', 'h'), '-', 'm'), '.', 's');
        sFile.timestamp = timestamp;
    end
end

if ~isempty(comments)
    ievt = 1;
    for icmt=1:length(comments)
        sFile.events(ievt).label = ['CMT ' comments(icmt).str];
        sFile.events(ievt).times = [comments(icmt).time comments(icmt).time];
        sFile.events(ievt).epochs = [comments(icmt).record comments(icmt).record];
        ievt = ievt + 1;
    end
    fprintf('Loaded %d comments as events\n', length(sFile.events));
end
